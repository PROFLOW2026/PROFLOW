...
describe('receivables summary', () => {
  ...
  it('nets credit notes into total Outstanding (not invoice gross alone)', () => {
    ...
    expect(summary.totalOutstanding.amount).toBe('800.000000');
    expect(summary.openCount).toBe(1);
  });
  ...
  it('documents credit/void integrity in the summary note', () => {
    ...
    expect(summary.note).toMatch(/Credit notes reduce Outstanding/i);
...
describe('receivables summary ↔ aging interaction', () => {
  it('aging total matches summary total for the same open outstanding set', () => {
    const openRecords = [
      ...
      record({
        ...
        dueDate: null,
        outstandingAmount: money('-40', currency),
        totalAmount: money('40', currency),
        collectionStatus: 'paid',
      }),
    ];

    const summary = computeReceivablesSummary(openRecords, currency, asOf);
    const aging = computeReceivablesAging(openRecords, currency, asOf);

    expect(aging.totalOutstanding.amount).toBe(summary.totalOutstanding.amount);
    expect(summary.totalOutstanding.amount).toBe('135.000000');
    expect(summary.overdueTotal.amount).toBe('150.000000');
    expect(aging.buckets.find((b) => b.key === 'current')?.total.amount).toBe('-15.000000');
  });
  ...
  it('void and draft never inflate summary or aging', () => {
    ...
    const summary = computeReceivablesSummary(records, currency, asOf);
    const aging = computeReceivablesAging(records, currency, asOf);
