...
describe('financial basis labels distinguish invoice cash from net profit', () => {
  it('he-IL invoiced vs profit strings are distinct without VAT ambiguity on billing KPIs', () => {
    ...
    expect(billedHint).toMatch(/מע״מ אינו חלק מהיתרת חוזה/);
    ...
    expect(profit).toMatch(/יתרת חוזה/);
    expect(actualMargin).toMatch(/יתרת חוזה/);
    expect(actualMarginHint).toMatch(/יתרת החוזה|עלויות מוכרות/);
    ...
    expect(paidHint).not.toMatch(/הכנסה ליתרת|הכנסה לרווח/);
