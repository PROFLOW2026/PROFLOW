...
describe('CommittedCost != Expense', () => {
  ...
  it('partially consumes commitment when AP match is less than open PO', () => {
    ...
    ).toEqual({ remainingAmount: '8000.000000', status: 'partially_consumed' });

  });



  it('closes commitment when AP match covers the open amount', () => {
    ...
    expect(
      ...
      computeCommittedAfterConsumption({
        ...
        consumeAmount: '100000',

        currency: 'ILS',

      }),

    ).toEqual({ remainingAmount: '0.000000', status: 'closed' });

  });



  it('closes commitment when match exceeds remaining open (over-receive)', () => {
