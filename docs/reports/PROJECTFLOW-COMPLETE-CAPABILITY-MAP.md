...
**HEAD:** `4b1099eb756d1b2ec366f274be054afeded710f8` (`Align BOQ documentation with released billing flow`)  
**Nature:** Read-only product mapping of the repository as it exists now. No features were implemented, no issues were fixed, no SQL was applied, no commit or push was made.  
**Portal:** OFF (public customer/vendor login is a hard product decision).
...
Companion file: [`PROJECTFLOW-CAPABILITY-MATRIX.csv`](./PROJECTFLOW-CAPABILITY-MATRIX.csv) (one row per capability).

---

## Owner summary — what ProjectFlow actually is today
...
- Organization, team, roles, Hebrew/RTL, business profiles
- Projects / jobs / work orders as one financial entity
- Expenses (draft → finalize → Actual) and corrections without silent rewrite
- Commercial contract: Original vs Current, pending vs approved changes
- Internal billing and customer payments (tracking, not tax invoices)
- Vendor bills, credits, payments, PO matching
- Workforce time → labor Actual, with employer-cost privacy
- Released BOQ: measure → approve → progress bill, without turning progress into Actual
- Today inbox, financial explainability, import/export, PWA shell + offline drafts

**Most complete workflows**

1. Owner signs up → creates a business → invites a team → opens a project → records expenses → sees Actual.
2. General contractor: contract → change orders → BOQ progress billing → customer payment → profitability.
3. Trade job: client → job → time/expense → invoice → payment.
4. Service: client → work order → dispatch → time/expense → complete → bill from the shared engine.

**Who it serves well today:** owners, project managers, and finance users of a general contractor, renovation firm, electrician, plumber, HVAC, or maintenance/field-service company. Workers can record hours, expenses, field logs, forms, and BOQ measurements without seeing profit or unit prices.
...
- Inventory, Gantt critical path, live API/webhooks, and banking are foundation or partial.
- AP bills have no VAT split, so VAT-inclusive vendor totals can inflate Actual.
- BOQ has no dedicated mobile field strip.

| Status | Count (master matrix) |
|---|---|
| FULL | **67** |
| OPTIONAL-FULL | **61** |
| PARTIAL | **14** |
| FOUNDATION | **15** |
| DISABLED | **3** |
| NOT-IMPLEMENTED (explicitly documented) | **15** |
| **Total mapped capabilities** | **175** |

---

## Financial truths the product actually enforces

These are not slogans. They are how the compose engine is written (`composeProjectFinancials`).

| Truth | Meaning for the owner |
|---|---|
| **Billing ≠ Payment** | An invoice is a billing record. Cash is a separate payment. Outstanding = invoiced − paid − held retention. |
| **Commitment ≠ Actual** | Issuing a purchase order creates committed cost. Actual appears when a vendor bill is posted (or an unmatched expense is finalized). |
| **Progress ≠ Actual** | Approving BOQ quantities does **not** create project cost. Only expenses, labor, and posted AP do. |
| **VAT ≠ Profit** | Current contract value and expense Actual are **net**. Profit = CCV − cost. Invoiced KPIs use **gross total**. |
| **Retention = cash timing** | Holdback reduces what is due now. It does **not** reduce contract value, invoiced, Actual, or profit. |

**Caveat (real gap):** vendor bills store only a total, with no net/tax split. If a bill is entered VAT-inclusive, that VAT can enter Actual on the AP path.
...
**Counted page files:** 132. **Redirect-only pages:** 5 (`/inbox` → `/today`, `/settings` → first allowed section, `/workforce` → `/workforce/employees`, `/portal/customer` and `/portal/vendor` → `/portal`). **Unique destinations:** 127. **Project tabs** (query `?tab=`) add extra screens on the same project/job page.
...
| `/jobs/new` | עבודה חדשה | `jobs` | `projects.create` | Create (client required) | Partial | OPTIONAL-FULL |
...
| `/work-orders/new` | קריאה חדשה | `service` | `service.manage` | Create | Partial | OPTIONAL-FULL |
...
| `/crm` | מכירות / הזדמנויות | `crm` | `crm.read/manage` | Pipeline table | Partial | OPTIONAL-FULL |
...
| `/field-ops/logs` `/new` `/[id]` | יומן שטח | `field_ops` | manage | Create, attach later | Partial | OPTIONAL-FULL |
| `/field-ops/punch` `/new` `/[id]` | ליקויים | `field_ops` | manage | Status/priority | Partial | OPTIONAL-FULL |
| `/field-ops/inspections` `/new` `/[id]` | ביקורות | `field_ops` | manage | Pass/fail | Partial | OPTIONAL-FULL |
...
| `/assets/inventory` `/[id]` | מלאי | `assets` | same | Qty movements | Partial | FOUNDATION |
...
**Not implemented:** document numbering settings, data-purge retention policy, project-scoped roles (column reserved).
...
**What it cannot do today:** a client 360 with AR aging, outstanding, or profit on the client card. CRM prospects are a **separate** record until conversion writes `convertedClientId`.
...
| Extra fields | — | Workers note at create is **free text**, not assignments | Status, priority, site, assignee, schedule |
...
Evidence: `drizzle/schema/projects.ts`, `createJob`, `createWorkOrder`, `convert-job-to-project.ts`.
...
**Moves CCV:** locking/correcting original (until lock); approving a change request.

**Does not move CCV:** pending CRs, BOQ create/progress/billing, AR invoices/payments/retention, display original, pre-sale quotes, Gantt % complete.

Tax: user may enter inclusive or exclusive; engine stores net/tax/gross and freezes a snapshot. Profit uses **net**.

Evidence: `drizzle/schema/contracts.ts`, `computeCurrentContractValue`, `approveChangeRequest`.

---

## 6. BOQ / Bill of Quantities

**Status: OPTIONAL-FULL.** Module key `boq`. Hebrew: **כתב כמויות / חשבונות חלקיים**. Enabled by General Contractor and Renovation profiles. Lives on the project tab, not main nav. **No dedicated mobile field path.**
...
7. Approved change orders may be **allocated** onto items (`current_*` recomputes). Pending COs cannot allocate.
...
12. Optional: subcontractor rate schedule → valuation → `proposed_ap`. The product **does not** auto-create a vendor bill, because creating a bill immediately posts **open** Actual.
...
Evidence: `drizzle/schema/boq.ts`, `src/modules/boq/**`, migrations 0032–0035, `tests/integration/boq/**`, Playwright `boq-happy-path.spec.ts` (not in CI).
...
Kinds: invoice, credit note, advance, retention release. Draft → finalize (immutable) → void/credit to correct. Payments only on finalized non-credit records. AR payments are **1:1** to one billing record (**PARTIAL** vs AP’s split applications). Statutory issuance is **DISABLED** until an external provider exists.
...
Actual = finalized expense NET
       ...
       + posted AP bill totals (minus bill-linked expenses)
...
| Vendor bill | Status open / partially_matched / matched (create already posts **open**) |
| Vendor credit | Reduces economic cost and outstanding; not a payment |
| PO | Never Actual — Commitment on issue |
| Subcontractor BOQ valuation | Never until someone posts AP manually |

**AP VAT gap:** bills have `total_amount` only.
...
Aging, project allocations, PO matches, documents, OCR → draft bill or draft credit: all exist. Subcontractor valuation stops at proposed AP.
...
**Status: OPTIONAL-FULL** for RFQ → quote compare → PO issue → commitment. **FOUNDATION** for receiving.
...
A `partially_received` status exists on POs. There is **no** receive-quantities application API and no receipt-line table.
...
| Inventory qty + receive/issue/adjust/return | FOUNDATION (`/assets/inventory`) |
| Project usage records | FOUNDATION (not Actual) |
| Locations, reservations, transfers, FIFO/AVG | NOT-IMPLEMENTED |
...
**Partial:** no dedicated work-order invoice object; billing uses shared AR. Checklist template id is **FOUNDATION** (not fully wired to Forms). Team assignee column was requested and is not in schema.
...
Exists: prospects + contacts, leads, opportunities (stage enum, table not kanban), notes, internal estimates, sales quotes with **immutable issued versions**, convert won deal → Client + Project + Contract from accepted quote.

**Does not exist:** activity timeline, follow-up reminders, kanban, automatic conversion without the explicit action.
...
**Gap:** BOQ allocations can reverse; there is **no** reverse-Change-Order API to undo CCV.
...
|---|---|---|
| Daily log (יומן שטח) | Site / supervisor | Text first; photos after save |
| Punch list (ליקויים) | Site / PM | open / in_progress / done / cancelled |
| Inspections (ביקורות) | QC on a project | scheduled → passed/failed — not HSE |
...
**Status: PARTIAL** as a cross-domain engine. Wired gates are real; two catalog types are unused.
...
| `quote_discount` | — | **Not wired** (FOUNDATION) |
| `time_correction` | — | Counted in month-close completeness only (FOUNDATION) |
...
**Status: OPTIONAL-FULL workflow, PARTIAL freeze.** This is **operational** close, not a statutory GL close.
...
App code blocks expense finalize/void/reversal, AP create/void, credits, bill allocations, and labor allocations when the month is closed (`assertMonthOpenForRewrite`). **There are no general database triggers** freezing operational rows (labor month-cost has a narrower trigger).
...
List pages also have local `?q=` filters. Custom fields are **not** searched.
...
**Status: OPTIONAL-FULL.** Entities: client, project, vendor, employee, opportunity, expense. Types: text, number, money, date, select, multi_select, boolean, reference. Required flag, archive, reserved keys block canonical money/status names. **Not** in search or reports.
...
| `overhead` | כלי תקורה עסקית | Auto/off | **None** | PARTIAL (no nav home) |
...
**Not fully supported:** CRM-on-by-default, receiving goods, auto AP from valuations, statutory invoice, GL close, customer portal approvals.
...
Supported: WO, dispatch, time, expense, complete, bill via shared AR. **Partial:** dedicated WO invoice, checklist wiring, inventory issue-to-job as Actual. Recurrence for maintenance contracts: OPTIONAL-FULL.

### D. Consultancy / design project

Client → project → assignments/time → expenses → milestone/billing → profitability

Supported: project, assignments, time as labor Actual, expenses, contract, billing (including advances), profit. **Not fully supported:** design-stage fee engine, drawing/BIM, retainer product, Gantt as MS Project, CRM follow-ups.
...
- Warehouse locations, stock reservations, transfers, inventory valuation
...
Schema or stub exists, complete owner workflow does not:
...
| PO receiving | Status enum | Receive qty API / receipt lines |
| Inventory | Qty movements | Locations, transfers, costing, Actual |
| Equipment usage | Records | Cost recognition |
| `overhead` module | Key + expense tools | Module home / nav |
| Project-scoped roles | `projectId` column | Seeding / UI |
| Quote-discount approval | Catalog type | Gate in quotes |
| Time-correction approval | Catalog type + month-close count | Gate on time entries |
| Contract `adjustment` events | Enum | App writer |
| Multi-contract | Schema | Primary-only UX |
| WO checklist | Column | Forms wiring |
| Notifications | Invite email port | Product |
| OCR queue | In-request extract | Durable worker |
| Month-close DB freeze | App asserts | Table triggers |
...
| Playwright e2e | `npm run test:e2e` — **not in CI** |
...
**Moderate:** OCR unit + some integration, CRM/quotes/service unit, planning unit, banking “matches do not mutate”, shell/i18n UI.

**Weak / absent from CI:** Playwright journeys (BOQ happy path, OCR review, owner/worker personas), webhook HTTP, statutory provider, live bank feed, public API breadth.

Migrations on disk: **0000–0035, numbering skips 0003** (journal 0002 → 0004). 35 SQL files.
...
|---|---|
| Identity / tenancy | Profiles, last-org hint, organizations, memberships, invitations, module prefs, JSON settings |
...
| Contracts / changes | Contract amounts, value events, CRs, CO quotes/versions, recorded approvals, change orders |
...
| Billing | Internal billing records/lines, customer payments |
| Tax | Dated rules, per-document overrides |
| Documents | Metadata + links (bytes in Storage) |
| CRM / quotes product | Prospects, leads, opportunities, sales quotes; separate pre-sale `estimates` |
...
| Procurement | Materials, RFQs, supplier quotes, POs, **committed_costs** |
| Field / assets | Logs, punch, inspections, assets, fleet, maintenance, inventory qty |
| AP | Bills, matches, payments/applications, credits, bill allocations |
...
The full 175-row table is in [`PROJECTFLOW-CAPABILITY-MATRIX.csv`](./PROJECTFLOW-CAPABILITY-MATRIX.csv).
...
## 49. Discovered product gaps

Recorded only. **Nothing was fixed during this mapping.**

### Financial-truth gaps (5)

| Area | Severity | Expected | Actual | Evidence | Affects |
|---|---|---|---|---|---|
| AP VAT | HIGH | Vendor Actual net of VAT, like expenses | Bills have only `total_amount`; recognition uses that total | `drizzle/schema/ap.ts`; compose AP sum | Financial truth |
| Invoiced vs profit VAT basis | MEDIUM | Operators may treat Invoiced as revenue | Invoiced is **gross**; profit uses **net CCV** | `signedBillingAmount` vs `computeProfitPosition` | Financial truth if unlabeled |
| Quote-discount approval | MEDIUM | Large discounts gated | Type exists; quotes never call the gate | `approvals/domain/types.ts` vs quotes app | Financial process |
| Time-correction approval | MEDIUM | Corrections gated | Type counted in month-close; no time-entry gate | month-close vs workforce | Financial process |
...
| BOQ base table | LOW | All reads via secure view | App uses `boq_nodes_secure`; raw table still has prices | `boq.repository.ts`; 0035 | Security if a future client hits the base table |

### Data-integrity gaps (3)

| Area | Severity | Expected | Actual | Evidence | Affects |
|---|---|---|---|---|---|
| Month-close DB freeze | MEDIUM | Closed period immutable at DB | App asserts; no general triggers | month-close SCHEMA_REQUEST | Data integrity |
| Change Order reversal | MEDIUM | Undo approved CO / CCV | BOQ allocation reverse only; no commercial reverse API | commercial vs `reverseBoqChangeAllocation` | Data integrity / financial |
| Document byte cleanup | LOW | Delete bytes with metadata | Soft-delete; storage remove best-effort | `softDeleteDocument` | Data (orphan objects) |

### Usability gaps (selected; 20+)

BOQ has no mobile field strip; PO receiving is status-only; subcontractor valuation cannot safely auto-draft AP; no client AR 360; CRM has no kanban/activities and is off in all profiles; three quote objects; no numbering settings; field photos after save; forms “signature” is acknowledgement; single reports page; webhooks/API do not leave the box; no notifications; email default off; custom fields absent from search/reports; OCR default off and in-request; overhead module has no home; job create “people” is a free-text note; AR payments cannot split across invoices; inventory is qty-only; Gantt is not MS Project; Settings country pack is IL/US/GB; portal Settings page is hidden; attendance completeness heuristic is weak; WO checklist not wired.

### Other product gaps

Google/AWS OCR unimplemented; live bank feed stub; multi-contract UX primary-only; contract `adjustment` kind unused; project-scoped roles unused; Playwright not in CI.
