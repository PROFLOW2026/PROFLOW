...
export async function getProjectLaborCost(
  ...
  // Open month: live preview replaces stored lines (no double count).
  // Closed current month: persisted applied/closed lines stay; preview is skipped.
  const currentMonthClosed = monthCostsReady
    ? await isMonthClosedForFinancialsRead(context, currentYearMonth)
    : false;
  ...
  const [aggregate, appliedPriorMonths, previewCurrentMonth] = await Promise.all([
    ...
    monthCostsReady
      ? sumMonthlyAllocatedLaborForProject(
          ...
          projectId,
          currency,
          currentMonthClosed ? undefined : { excludeYearMonths: [currentYearMonth] },
        )
      : Promise.resolve({
          projectId,
          totalAmount: '0',
    ...
    monthCostsReady && !currentMonthClosed
