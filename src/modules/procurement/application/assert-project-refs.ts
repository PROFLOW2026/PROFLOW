...
/**
 * App-layer tenant guard (RLS is defense in depth only).
 * Ensures optional project / work-package FKs stay inside the active organization.
 */
export async function assertOptionalProjectRefsInOrg(
  context: OrgContext,
  input: { projectId?: string | null; workPackageId?: string | null },
): Promise<void> {
  if (input.workPackageId && !input.projectId) {
    throw new DomainRuleError(
      'projectId is required when workPackageId is set',
