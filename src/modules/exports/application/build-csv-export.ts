...
async function tableBilling(context: OrgContext, copy: ExportCopy): Promise<ExportTable> {
  ...
  return {
    ...
    headers: [
      ...
      h(copy, 'dueDate'),
      h(copy, 'currency'),
      h(copy, 'subtotalAmount'),
      h(copy, 'taxAmount'),
      h(copy, 'totalAmount'),
      h(copy, 'paidAmount'),
      h(copy, 'paidGrossAmount'),
      h(copy, 'outstandingAmount'),
      h(copy, 'outstandingGrossAmount'),
    ],
    rows: items.map((item) => [
      item.id,
      item.projectId,
      ...
      toExcelNumber((item.subtotalAmount ?? item.totalAmount).amount),
      toExcelNumber(item.taxAmount?.amount ?? '0'),
      toExcelNumber(item.totalAmount.amount),
      toExcelNumber(item.paidAmount.amount),
      toExcelNumber((item.paidGrossAmount ?? item.paidAmount).amount),
      toExcelNumber(item.outstandingAmount.amount),
      toExcelNumber((item.outstandingGrossAmount ?? item.outstandingAmount).amount),
    ]),
...
async function tableProjectFinancials(
  ...
  rows.push(
    ...
    [
      copy.metrics.paid,
      toExcelNumber(financials.billing.netPaid.amount),
      financials.billing.netPaid.currency,
      `Gross: ${financials.billing.paid.amount}`,
    ],
    [
      copy.metrics.outstanding,
      toExcelNumber(financials.billing.netOutstanding.amount),
      financials.billing.netOutstanding.currency,
      `${copy.metricNotes.derived}; Gross: ${financials.billing.outstanding.amount}`,
    ],
    [
