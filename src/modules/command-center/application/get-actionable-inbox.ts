...
/**
 * Shared actionable source for Today and the notification bell.
 * Same collectors, persona filter, and snooze/handle state as /today.
 */
export async function getActionableInbox(context: OrgContext): Promise<CommandCenterInbox> {
...
export async function getActionableInboxIfAllowed(
  context: OrgContext,
): Promise<CommandCenterInbox | null> {
  if (!hasPermission(context, PERMISSIONS.COMMAND_CENTER_READ)) return null;
