...
export interface PostponeMenuProps {
  readonly dueDate: string | null;
  readonly today: string;
  readonly compact?: boolean;
  readonly disabled?: boolean;
  readonly triggerClassName?: string;
  readonly onApply: (nextDueDate: string) => void | Promise<void>;
}
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            ...
            className={cn(
              compact
                ? 'rounded-md border border-[var(--pf-border-default)] bg-[var(--pf-bg-surface)] px-2 py-1 text-xs font-medium hover:bg-[var(--pf-bg-muted)]'
                : 'inline-flex min-h-9 items-center rounded-md border border-[var(--pf-border-default)] bg-[var(--pf-bg-surface)] px-3 py-1.5 text-sm font-medium hover:bg-[var(--pf-bg-muted)]',
              triggerClassName,
              'shrink-0',
            )}
