...
import { upsertEntityFieldValueAction } from '@/app/[locale]/(app)/settings/custom-fields/actions';
import { ExpenseCorrectionHistory } from '@/app/[locale]/(app)/expenses/[expenseId]/expense-correction-history';
import { ExpenseDetailActions } from '@/app/[locale]/(app)/expenses/[expenseId]/expense-detail-actions';
import { ExpenseEditForm } from '@/app/[locale]/(app)/expenses/[expenseId]/expense-edit-form';
...
interface ExpensesOrgDetailPageProps {
...
export async function ExpensesOrgDetailPage({
  expenseId,
  routeBase,
  surface = 'owner',
  ...
  const data = await withOrgContext(async (context) => {
    if (surface === 'employee') {
      ...
      if (!orgListHasPermission(context, PERMISSIONS.EXPENSES_READ, surface)) {
        return null;
      }
    }
    try {
      ...
      const workPackages = expense.projectId
        ...
        : [];
      const vendors = orgListHasPermission(context, PERMISSIONS.VENDORS_READ, surface)
        ? await listVendorsForOrg(context, { status: 'active' }).catch(() => [])
        : [];
      const inventoryItems = orgListHasPermission(context, PERMISSIONS.ASSETS_MANAGE, surface)
      ...
      return {
        ...
        canPromoteVendor: orgListHasPermission(context, PERMISSIONS.VENDORS_MANAGE, surface),
        canFinalizeExpense: orgListHasPermission(context, PERMISSIONS.EXPENSES_FINALIZE, surface),
        canCreateExpense: orgListHasPermission(context, PERMISSIONS.EXPENSES_CREATE, surface),
        canUpdateExpense: orgListHasPermission(context, PERMISSIONS.EXPENSES_UPDATE, surface),
        monthOpen:
  ...
  return (
    <div className="mx-auto flex min-w-0 w-full max-w-2xl flex-col gap-6">
          ...
          <CardContent className="grid gap-3 text-sm">
            ...
            {expense.vendorId ? (
              <div className="grid gap-0.5">
                ...
                <Link href={`${vendorsRouteBase}/${expense.vendorId}`} className={textNavLinkClassName}>
                  {t('detail.viewVendor')}
                </Link>
              </div>
            ) : null}
      ...
      <EntityCustomFieldsPanel
        entityId={expense.id}
        fields={customFields}
        revalidatePath={`${routeBase}/${expense.id}`}
